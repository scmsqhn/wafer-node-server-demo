...
module.exports = (req, res) => {
	...
	if (codedata.code == "1001") { //obtains goodsList
		Step.Step(function (result, entire) {
			...
			//console.log(entire); // []
			var that = this
            console.log(JSON.stringify(codedata.data))
			sync.findDataFromMongo("goodsList", 
                JSON.parse(codedata.data)
				, that.step)
		}, function (result, entire) {
			console.log("FINAL RESULT IS====>"+JSON.stringify(result)); // 'abc'
			//console.log(entire); // ['abc']
            console.log('RESULT 200')
			res.writeHead(200, {'Content-Type': 'application/json'});
			res.end(JSON.stringify(result));
            return 0            //return ::返回一个非undefined的值，和调用this.step();效果相同
